...
import { uploadData, getAllDevices } from "../api/Devices";
...
const Home = () => {
      ...
      render: (text, record) => {
        return (
          <a
            onClick={() => {
              //setting form data
              let _formData = data.find((d) => d.deviceSerial == text);
  ...
  const handleFileUpload = async (e) => {
    try {
      ...
      let result = ExcelToJson(e.target.files[0], async (excelData) => {
